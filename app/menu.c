    ...
    *pMin = 0;
    ...
    switch (menu_id)
    {
        ...
        case MENU_CW_TONE:
            ...
            *pMax = MORSE_TONE_HZ_MAX / MORSE_TONE_HZ_STEP;
            break;

        case MENU_CW_BEEP:
            *pMin = MORSE_BEEP_INTERVAL_MIN_MS / 500u;
            *pMax = MORSE_BEEP_INTERVAL_MAX_MS / 500u;
            break;
...
//          gRequestSaveChannel = 1;
        ...
        case MENU_CW_TONE:
        {
            uint32_t tone_hz = (uint32_t)gSubMenuSelection * MORSE_TONE_HZ_STEP;
            if (tone_hz < MORSE_TONE_HZ_MIN)
                tone_hz = MORSE_TONE_HZ_MIN;
            if (tone_hz > MORSE_TONE_HZ_MAX)
                tone_hz = MORSE_TONE_HZ_MAX;
            morse_tone_hz = (uint16_t)tone_hz;
            break;
        }

        case MENU_CW_BEEP:
        {
            uint32_t beep_ms = (uint32_t)gSubMenuSelection * 500u;
            if (beep_ms > MORSE_BEEP_INTERVAL_MAX_MS)
                beep_ms = MORSE_BEEP_INTERVAL_MAX_MS;
            morse_beep_ms = (uint16_t)beep_ms;
            break;
        }
...
void MENU_ShowCurrentSetting(void)
{
    switch (UI_MENU_GetCurrentMenuId())
    {
        ...
        case MENU_CW_BEEP:
            gSubMenuSelection = morse_beep_ms / 500u;
            break;
